...
int main() {
    ...
    for (int platformIndex = 0; platformIndex < platformsCount; ++platformIndex) {
        ...
        for (int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex) {
            ...
            OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_VENDOR_ID, sizeof(device_vendor_id), &device_vendor_id, nullptr));
            std::cout << "        Device Vendor ID: " << device_vendor_id << std::endl;
            // - CL_DEVICE_OPENCL_C_VERSION
            size_t clDeviceOpenCLCVersionNameSize = 0;
            OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_OPENCL_C_VERSION, 0,nullptr, &clDeviceOpenCLCVersionNameSize));
            std::vector<unsigned char> clDeviceOpenCLCVersionName(clDeviceOpenCLCVersionNameSize, 0);
            OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_OPENCL_C_VERSION,clDeviceOpenCLCVersionNameSize ,clDeviceOpenCLCVersionName.data(), nullptr));
            std::cout << "        Device OpenCL C Version:  " << clDeviceOpenCLCVersionName.data() << std::endl;
